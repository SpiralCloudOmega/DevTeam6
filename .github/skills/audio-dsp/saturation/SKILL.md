...
- **Frequency-dependent**: HF saturates first, producing natural rolloff under drive.
- **Transient softening**: compresses peaks gently, adding glue and cohesion.
- The **Jiles-Atherton** model (CHOWTapeModel) is the standard for realistic tape emulation.
...
The most critical consideration for nonlinear DSP. New harmonics above Nyquist (sampleRate / 2) fold back as inharmonic aliasing. Solution: upsample → saturate → downsample with anti-aliasing. Typical: 2×–8×.
...
Different curves for positive/negative half-cycles generate even harmonics (2nd, 4th, 6th…). Tubes and single-ended circuits are inherently asymmetric. Always add a DC blocker after.

## Mix / Blend Control
Dry/wet mix is essential — dial from subtle warmth to heavy destruction.
